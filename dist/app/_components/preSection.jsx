...
export default function PreLaunchSection() {
    ...
    return (<section id='mecs' className="w-full pb-12 md:pb-24 lg:pb-32 bg-gradient-to-r from-primary to-primary-dark">
      <div className="container px-4 md:px-6 mx-auto">
        <div className="flex flex-col items-center space-y-4 text-center">
          ...
          {!isSubmitted ? (<form onSubmit={handleSubmit} className="w-full max-w-md space-y-4">
              <div className="flex flex-col sm:flex-row gap-2">
                <input type="tel" placeholder="Seu melhor telefone" value={cell} onChange={(e) => setCell(e.target.value)} required className="flex-grow px-4 py-3 rounded-md text-white bg-black border border-primary-200 focus:outline-none focus:ring-2 focus:ring-primary-100"/>
                <button type="submit" className="px-6 py-3 bg-secondary text-black rounded-md hover:bg-secondary-dark transition-colors duration-300 flex items-center justify-center">
                  <Send className="w-5 h-5 mr-2"/>
                  Inscrever-se
                </button>
              </div>
              {error && <p className="text-red-500">{error}</p>}
            </form>) : (<div className="bg-black text-white px-6 py-4 rounded-md shadow-lg animate-fade-in-up">
